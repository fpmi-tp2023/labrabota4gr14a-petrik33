...
- [Mobile Software Programming Technologies. Lab Work №4](#mobile-software-programming-technologies-lab-work-4)
  - [Overview](#overview)
    - [Задание 1. Установка sqlite в macOS](#задание-1-установка-sqlite-в-macos)
      - [Способ 1. Установить sqlite из исходников](#способ-1-установить-sqlite-из-исходников)
      - [Способ 2. Установить с помощью утилиты Homebrew](#способ-2-установить-с-помощью-утилиты-homebrew)
      - [Способ 3. Установить из порта](#способ-3-установить-из-порта)
    - [Задание 2. Управление базой данных из консоли](#задание-2-управление-базой-данных-из-консоли)
      - [Упражнение 2.1. Изучить видео](#упражнение-21-изучить-видео)
      - [Упражнение 2.2. Изучить примеры](#упражнение-22-изучить-примеры)
      - [Упражнение 2.3. Создать базу данных и выполнить запросы](#упражнение-23-создать-базу-данных-и-выполнить-запросы)
      - [Упражнение 2.4. Выполнить запросы по вариантам](#упражнение-24-выполнить-запросы-по-вариантам)
    - [Задание 3. Управление базой данных в SQLite Database Manager](#задание-3-управление-базой-данных-в-sqlite-database-manager)
      - [Упражнение 3.1. Изучить примеры](#упражнение-31-изучить-примеры)
      - [Упражнение 3.2. Выполнить учебные запросы по вариантам](#упражнение-32-выполнить-учебные-запросы-по-вариантам)
      - [Упражнение 3.3. Создать БД и выполнить запросы по вариантам](#упражнение-33-создать-бд-и-выполнить-запросы-по-вариантам)
    - [Задание 4. Изучение примеров приложений на C подключения и запросов к базе данных](#задание-4-изучение-примеров-приложений-на-c-подключения-и-запросов-к-базе-данных)
      - [Дополнительные инструкции](#дополнительные-инструкции)
    - [Задание 5. Разработка спецификации проекта на языке UML](#задание-5-разработка-спецификации-проекта-на-языке-uml)
  - [Specification: ID 34](#specification-id-34)
    - [Задание 2](#задание-2)
      - [Упражнение 2.3](#упражнение-23)
      - [Упражнение 2.4](#упражнение-24)
    - [Задание 3](#задание-3)
      - [Упражнение 3.2](#упражнение-32)
  - [Usage](#usage)
  - [Key Questions](#key-questions)
    - [1. Перечислите способы создания базы данных sqlite](#1-перечислите-способы-создания-базы-данных-sqlite)
    - [2. С помощью какой команды в консоли sqlite можем просмотреть список баз данных и подключенных файлов баз данных?](#2-с-помощью-какой-команды-в-консоли-sqlite-можем-просмотреть-список-баз-данных-и-подключенных-файлов-баз-данных)
    - [3. Приведите перечень команд для экспорта данных из таблицы базы данных в файл с расширением .csv](#3-приведите-перечень-команд-для-экспорта-данных-из-таблицы-базы-данных-в-файл-с-расширением-csv)
    - [4. Приведите перечень команд для экспорта отдельной таблицы и всей базы данных в файл с расширением .sql и сжатый файл, например в файл с расширением .sql.tgz](#4-приведите-перечень-команд-для-экспорта-отдельной-таблицы-и-всей-базы-данных-в-файл-с-расширением-sql-и-сжатый-файл-например-в-файл-с-расширением-sqltgz)
      - [Вся база данных:](#вся-база-данных)
      - [Таблица:](#таблица)
      - [Сжатие:](#сжатие)
    - [5. Как вывести из таблицы данные по строкам и по столбцам?](#5-как-вывести-из-таблицы-данные-по-строкам-и-по-столбцам)
    - [6. Для чего используется команда .headers в консоли sqlite?](#6-для-чего-используется-команда-headers-в-консоли-sqlite)
    - [7. Какая команды используется для вывода настроек окружения в sqlite?](#7-какая-команды-используется-для-вывода-настроек-окружения-в-sqlite)
    - [8. С помощью какой команды выводится список таблиц базы данных в консоли sqlite?](#8-с-помощью-какой-команды-выводится-список-таблиц-базы-данных-в-консоли-sqlite)
    - [9. Приведите пример запроса выборки из 2-х таблиц](#9-приведите-пример-запроса-выборки-из-2-х-таблиц)
    - [10. Приведите пример запроса для обновления данных в строках таблицы в зависимости от значения определённого поля](#10-приведите-пример-запроса-для-обновления-данных-в-строках-таблицы-в-зависимости-от-значения-определённого-поля)
    - [11. Приведите пример функции, которая открывает соединение с файлом базы данных SQLite и возвращает объект соединения с базой данных, который будет использоваться другими функциями SQLite?](#11-приведите-пример-функции-которая-открывает-соединение-с-файлом-базы-данных-sqlite-и-возвращает-объект-соединения-с-базой-данных-который-будет-использоваться-другими-функциями-sqlite)
    - [12. Приведите пример синтаксиса функции sqlite3\_exec и объясните результат выполнения](#12-приведите-пример-синтаксиса-функции-sqlite3_exec-и-объясните-результат-выполнения)
    - [13. Какая функция закрывает соединение с базой данных, ранее открытое вызовом sqlite3\_open? Приведите пример синтаксиса](#13-какая-функция-закрывает-соединение-с-базой-данных-ранее-открытое-вызовом-sqlite3_open-приведите-пример-синтаксиса)
    - [14. Приведите пример фрагмента кода на языке C для создания таблицы в базе данных sqlite и объясните его](#14-приведите-пример-фрагмента-кода-на-языке-c-для-создания-таблицы-в-базе-данных-sqlite-и-объясните-его)
    - [15. Приведите пример фрагмента кода на языке C для вставки данных в таблицу в базе данных sqlite и объясните его](#15-приведите-пример-фрагмента-кода-на-языке-c-для-вставки-данных-в-таблицу-в-базе-данных-sqlite-и-объясните-его)
    - [16. Приведите пример фрагмента кода на языке C выполнением AUTOCOMMIT и TRANSACTION и объясните в чем особенности использования их](#16-приведите-пример-фрагмента-кода-на-языке-c-выполнением-autocommit-и-transaction-и-объясните-в-чем-особенности-использования-их)
